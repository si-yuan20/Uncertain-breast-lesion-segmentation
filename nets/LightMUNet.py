...
def create_model(model_type="lightmunet", num_classes=1, num_channels=1, **kwargs):
    """
    Create a LightMUNet model
    
    Args:
        model_type: str, model type ("lightmunet")
        num_classes: int, number of output classes
        num_channels: int, number of input channels
        **kwargs: additional parameters
    
    Returns:
        nn.Module: LightMUNet model
    """
    if model_type == "lightmunet":
        # LightMUNet is designed for 2D or 3D images, default to 2D for medical imaging segmentation
        return LightMUNet(
            spatial_dims=2,
...
if __name__ == "__main__":
    ...
    model = create_model("lightmunet", num_classes=1, num_channels=1).to(device)
